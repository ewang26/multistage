...
def model_training(model, num_epochs, lr_factor=1, optimizer_pth=None, save_opt=True, order=None):
    train_losses = []
    test_losses = []
...
# This model (with name "v2") will be the result of retraining the first stage from scratch
torch.save(model1.state_dict(), 'models/E1000_D1_3L_AFrelu_1S_v2.pth')
...
# %%
epsilon = compute_mse(train_dataloader, model1)[0]
rmse_val = epsilon**0.5

# %%
# This will load in the previous optimizer for continued training
model1.load_state_dict(torch.load('models/E1000_D1_3L_AFrelu_1S_v2.pth'))
train_losses, test_losses = model_training(model1, 1000, lr_factor=epsilon,\
    optimizer_pth="models/E1000_D1_3L_AFrelu_1S_optimizer.pth", save_opt=False, order=1)

# This model will be the result of continued training of the first stage
torch.save(model1.state_dict(), 'models/E1000_D1_3L_AFrelu_1S_continued_v3.pth')

# %%
print(f"MSE over all train functions: {compute_mse(train_dataloader, model1)[0]}")
print(f"MSE over all test functions: {compute_mse(test_dataloader, model1)[0]}")

# %%
plot_losses(train_losses=train_losses, test_losses=test_losses,\
    save_dir='plots/continued', filename='D1_3L_AFrelu_1S_continued_loss_v3', save=save)
