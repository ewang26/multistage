...
# %%
function_derivative_pairs_train = get_inverse_predictions(model1, train_dataloader)
function_derivative_pairs_test = get_inverse_predictions(model1, test_dataloader)

# %%
dataiter = iter(function_derivative_pairs_train)
function, deriv = next(dataiter)
print(deriv.shape)
...
def inverse_model_training(model, train_dataloader, test_dataloader, num_epochs, order=None):
    ...
    for epoch in range(num_epochs):
        ...
        for functions, model_derivatives in train_dataloader:
        ...
        model.eval()
        with torch.no_grad():
            for test_functions, test_model_derivatives in test_dataloader:
...
print(f"Train MSE: {compute_mse(function_derivative_pairs_train, inverse_model)[0]},\
    Train NMSE: {compute_mse(function_derivative_pairs_train, inverse_model)[1]}")

print(f"Test MSE: {compute_mse(function_derivative_pairs_test, inverse_model)[0]},\
    Test NMSE: {compute_mse(function_derivative_pairs_test, inverse_model)[1]}")
